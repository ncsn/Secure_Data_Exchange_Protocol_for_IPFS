 ...
 * ── Privacy protocol ──────────────────────────────────────────────────────────
 *
 *   When a node calls requestPrivate(cid3, cid1, K, ownerConn):
 *     Step 1: send WANT_HAVE(cid3)
 *     Step 2: receive PRIVACY_CHALLENGE(sig)
 *             verify sig against owner's public key
 *     Step 3: send PRIVACY_RESPONSE(ecies(cid1 + K, ownerPubKey))
 *     Step 4: receive PRIVACY_BLOCK(aes_K(OBJ))
 *             decrypt with K, verify hash = cid1 digest
 *
 *   When a node is the owner and receives WANT_HAVE(cid3):
 *     - looks up cid3 → cid2 mapping (owner's private table)
 *     - sends PRIVACY_CHALLENGE(sign(cid2))
 *     - waits for PRIVACY_RESPONSE, decrypts to get cid1 + K
 *     - verifies cid1 matches, fetches OBJ, encrypts with K, sends PRIVACY_BLOCK
 */
...
import { sign, verify, eciesDecrypt, eciesEncrypt,
         aesEncrypt, aesDecrypt }                       from '../libp2p/crypto.js';
...
export class BitswapEngine extends EventEmitter {
  ...
  /**
   * requestPrivate(cid3String, cid1Digest, K, conn) → Promise<Buffer>
   *
   ...
   */
  requestPrivate(cid3String, cid1Digest, K, conn) {
    return new Promise((resolve, reject) => {
      if (this._pending.size >= MAX_PENDING) {
        return reject(new Error('Too many pending requests'));
      }
      ...
      this._pending.set(cid3String, {
        ...
        conn,
        timeout,
        // Flag as a privacy request so _handleMessage routes it correctly
        isPrivate: true,
        cid1Digest,
        K,
        resolve,
  ...
  async _handleMessage(msg, conn) {
    ...
    if (type === MessageType.WANT_HAVE) {
      if (this._owned.has(cid)) {
        // Privacy request — Step 2: send sign(CID² || nonce)
        // The nonce ensures freshness: each challenge produces a unique signature,
        // preventing replay of a captured PRIVACY_CHALLENGE.
        const owned = this._owned.get(cid);
        const nonce = randomBytes(32);
        const signedData = Buffer.concat([owned.cid2.digest, nonce]);
        const sig   = sign(signedData, this.localPeer.privateKey);

        // Pack: [ nonce(32) | sig_length(2) | sig | pubKey_length(2) | pubKey ]
        const pubKey  = this.localPeer.publicKeyRaw;
        const packed  = Buffer.alloc(32 + 2 + sig.length + 2 + pubKey.length);
        let off = 0;
        nonce.copy(packed, off);                off += 32;
    ...
    if (type === MessageType.PRIVACY_RESPONSE) {
      ...
      try {
        ...
        if (decrypted.length >= DECOY_FLAG.length &&
          ...
          // Use PRIVACY_BLOCK — not DECOY_BLOCK — so the response is also
          // indistinguishable from a real block transfer on the wire
          this._send(conn, MessageType.PRIVACY_BLOCK, cid, randomData);
          ledger.bytesSent += randomData.length;
          return;
        }

        // Real privacy request — first 32 bytes = CID¹ digest, rest = AES key K
        ...
        if (verifiedH2.length !== owned.cid2.digest.length ||
          ...
          // Requester doesn't know CID¹ — send DONT_HAVE so they fail fast
          // instead of hanging until timeout. Do not crash the engine.
          this._send(conn, MessageType.DONT_HAVE, cid);
          return;
        }

        // Fetch raw data from block store (not cached in _owned — avoids unbounded memory)
        ...
        // Encrypt OBJ with K and send PRIVACY_BLOCK
        const encrypted = aesEncrypt(rawData, K);
        this._send(conn, MessageType.PRIVACY_BLOCK, cid, encrypted);
        ledger.bytesSent += encrypted.length;

      } catch (err) {
        // Decryption failed (malformed payload, wrong key, etc.) — reject silently
        this._send(conn, MessageType.DONT_HAVE, cid);
      }
      return;
    }
    ...
    if (type === MessageType.PRIVACY_CHALLENGE) {
      ...
      if (decoyPending?.isDecoy) {
        ...
        // Extract B's public key from the challenge payload
        // Format: [ nonce(32) | sig_len(2) | sig | pubKey_len(2) | pubKey ]
      ...
      // Unpack: [ nonce(32) | sig_length(2) | sig | pubKey_length(2) | pubKey ]
      let off       = 0;
      const nonce   = payload.slice(off, off + 32); off += 32;
      const sigLen  = payload.readUInt16BE(off); off += 2;
      if (off + sigLen + 2 > payload.length) {
        clearTimeout(pending.timeout);
        this._pending.delete(cid);
      ...
      // authenticated during the TCP handshake. Prevents a MITM substituting a
      // different public key to intercept the ECIES-encrypted CID¹ + K in Step 3.
      const expectedKey = conn.remotePeer.publicKeyRaw;
      if (pubKeyRaw.length !== expectedKey.length ||
          !timingSafeEqual(pubKeyRaw, expectedKey)) {
        clearTimeout(pending.timeout);
      ...
      // SECURITY: verify the signature over (CID² digest || nonce) to confirm the
      // owner actually knows CID² (proof of possession) AND that this challenge is
      // fresh (nonce prevents replay of captured signatures).
      const expectedCid2Digest = hash(pending.cid1Digest);
      const signedData = Buffer.concat([expectedCid2Digest, nonce]);
      ...
      // Step 3: encrypt (CID¹ digest + K) with the owner's authenticated public key.
      // Use conn.remotePeer.publicKey (set during TCP handshake) — already verified above.
      this.emit('handshake:step', { step: 2, total: 4, peerId: conn.remotePeer.id, cid, message: 'PRIVACY_CHALLENGE verified \u2014 signature OK' });

      const packed    = Buffer.concat([pending.cid1Digest, pending.K]);
      const encrypted = eciesEncrypt(packed, conn.remotePeer.publicKey);

      this._send(conn, MessageType.PRIVACY_RESPONSE, cid, encrypted);
      this.emit('handshake:step', { step: 3, total: 4, peerId: conn.remotePeer.id, cid, message: 'PRIVACY_RESPONSE sent (encrypted CID\u00b9 + K)' });
      ledger.bytesSent += encrypted.length;
      return;
    }
